...
// utf-8 输出契约(ascii 转义会让 bundle 白涨 ~15KB,挤占 256 KiB 单文件上限)。
{
  ...
  assert.ok(/function GatewayQuotaBox[\s\S]*miniMaxRow\(name, win, direction, t\)/.test(src96), 'issue #96:网关卡片行渲染复用 miniMaxRow(与计划卡同口径)')
...
// 20) Gateway quota 纯 parser / registry 契约(issue #87):全部 fixture 内联、无网络。
{
  const gateway = await import('../lib/gateway-quota-adapters.js')
  const {
    parseAntigravityQuota, parseClaudeUsage, parseCodexUsage, parseKimiUsage,
    parseXaiBilling, parseWorkBuddyCredits, canonicalGatewayProvider,
    GATEWAY_PROVIDER_ADAPTERS, ANTIGRAVITY_QUOTA_URLS, ANTIGRAVITY_REQUEST_HEADERS,
    CLAUDE_REQUEST_HEADERS, CODEX_REQUEST_HEADERS, KIMI_REQUEST_HEADERS,
    XAI_REQUEST_HEADERS,
  } = gateway
  const fakeNow = Date.parse('2026-09-02T00:00:00.000Z')

  const ag = parseAntigravityQuota({ groups: [
    { group_id: 'main', buckets: [
      { window: '5h', remainingFraction: 0, resetTime: 1788350400 },
      { window: 'weekly', remainingFraction: 0.28 },
      { window: 'weekly', remainingFraction: 0.72 },
      { window: 'unknown-window', remainingFraction: 1 },
      { window: 'bad', remainingFraction: 28 },
    ] },
    { group_id: 'secondary', buckets: [{ window: '5h', remainingFraction: '72%' }] },
    { group_id: 'empty', buckets: [{ window: '5h', remainingFraction: null }] },
  ] })
  assert.equal(ag.windows.find(w => w.id === 'main:five-hour').percent, 100)
  assert.equal(ag.windows.find(w => w.id === 'main:weekly').percent, 72)
  assert.equal(ag.windows.filter(w => w.id.endsWith(':weekly')).length, 1, '重复 group 内同名窗口不覆盖')
  assert.equal(ag.windows.find(w => w.id === 'secondary:five-hour').percent, 28)
  assert.equal(ag.windows.some(w => w.id === 'main:unknown-window'), true)
  ...
  const claude = parseClaudeUsage({
    ...
    seven_day_sonnet: { utilization: 50 }, seven_day_cowork: { utilization: 60 },
    iguana_necktie: { utilization: 70 }, ignored: { resets_at: 'bad' },
    limits: [
      { kind: 'WEEKLY_SCOPED', is_active: false, percent: 20, scope: { model: { display_name: 'Fable' } } },
      { kind: 'weekly_scoped', is_active: true, percent: 33, resets_at: 1788350400, scope: { model: { display_name: 'fable 5' } } },
    ],
  })
  assert.equal(claude.windows.length, 7)
  assert.equal(claude.windows.find(w => w.id === 'seven-day-fable').percent, 33)
  assert.equal(claude.windows.find(w => w.id === 'seven-day-fable').scope, 'model')
  assert.equal(parseClaudeUsage({ five_hour: { utilization: 140 } }).windows[0].percent, 100)

  const codex = parseCodexUsage({
    plan_type: ' Pro ',
    rate_limit: { primary_window: { used_percent: 12, limit_window_seconds: 18000, reset_at: '2026-09-02T01:00:00Z', reset_after_seconds: 9 }, secondary_window: { used_percent: 34, limit_window_seconds: 604800 } },
    code_review_rate_limit: { primary_window: { limit_window_seconds: 2592000, limit_reached: true } },
    additional_rate_limits: [{ limit_name: 'Images', rate_limit: { primary_window: { used_percent: 5, limit_window_seconds: 18000 } } }],
  }, { now: () => fakeNow })
  assert.equal(codex.plan, 'pro')
  assert.equal(codex.windows.find(w => w.id === 'primary:five-hour').percent, 12)
  assert.equal(codex.windows.find(w => w.id === 'primary:five-hour').resetsAt, '2026-09-02T01:00:00.000Z')
  assert.equal(codex.windows.find(w => w.id === 'primary:weekly').periodHours, 168)
  assert.equal(codex.windows.find(w => w.id.startsWith('code-review:')).percent, 100)
  assert.equal(codex.windows.find(w => w.id.startsWith('additional-')).scope, 'model')

  const kimi = parseKimiUsage({ limits: [
    { name: 'Kimi 5h', limit: 100, remaining: 40, window: { duration: 5, timeUnit: 'TIME_UNIT_HOUR' }, reset_in: 60 },
    { name: 'Unknown minutes', limit: 100, remaining: 50, window: { duration: 30, timeUnit: 'UNKNOWN_UNIT' } },
    { name: 'Weekly fallback', limit: 100, used: 10 },
  ], usage: { limit: 200, remaining: 100 } }, { now: () => fakeNow })
  assert.equal(kimi.windows.find(w => w.label === 'Kimi 5h').percent, 60)
  assert.equal(kimi.windows.find(w => w.label === 'Kimi 5h').periodHours, 5)
  assert.equal(kimi.windows.find(w => w.label === 'Kimi 5h').resetsAt, '2026-09-02T00:01:00.000Z')
  assert.equal(kimi.windows.find(w => w.label === 'Unknown minutes').periodHours, 0.5)
  assert.equal(kimi.windows.some(w => w.label.includes('周限额')), true)

  const xai = parseXaiBilling({
    weekly: { creditUsagePercent: 25, currentPeriod: { type: 'weekly', start: '2026-09-01T00:00:00Z', end: '2026-09-08T00:00:00Z' }, productUsage: [{ product: 'models', usage_percent: 10 }] },
    monthly: { monthlyLimit: { val: 1000 }, used: { val: 250 }, currentPeriod: { type: 'monthly', start: '2026-09-01T00:00:00Z', end: '2026-10-01T00:00:00Z' }, onDemandCap: 100, onDemandUsed: 20 },
  })
  assert.equal(xai.windows.find(w => w.id === 'billing-weekly').percent, 25)
  assert.equal(xai.windows.find(w => w.id === 'billing-weekly').periodHours, 168)
  assert.equal(xai.windows.find(w => w.id === 'on-demand').percent, 20)
  assert.equal(xai.windows.some(w => w.scope === 'product'), true)
  assert.equal(parseXaiBilling({ weekly: null, monthly: { monthlyLimit: 1000, used: 250, currentPeriod: { type: 'monthly', start: '2026-09-01T00:00:00Z', end: '2026-10-01T00:00:00Z' } } }).windows.find(w => w.id === 'billing-monthly').percent, 25)

  const workbuddy = parseWorkBuddyCredits({ accounts: [
    { auth_index: 'wb-1', nickname: 'Real Person', uid: 'real-uid', credits: { total_remain: 380, total_used: 120, total_size: 500, packages: [{ name: 'Personal', remain: 380, used: 120, size: 500, cycle_start: '2026-09-01T00:00:00Z', cycle_end: '2026-10-01T00:00:00Z' }] } },
    { auth_index: 'wb-2', nickname: 'Other', uid: 'uid-2', credits: { total_remain: 0, total_used: 0, total_size: 0, packages: [] } },
    { auth_index: 'wb-3', error: 'plugin account unavailable' },
  ] })
  assert.equal(workbuddy.accounts[0].windows[0].percent, 24)
  assert.equal(workbuddy.accounts[0].credits.fetchedAt, '')
  assert.equal(workbuddy.accounts[0].credits.packages[0].resetsAt, '2026-10-01T00:00:00.000Z')
  assert.equal(workbuddy.accounts[1].status, 'unknown')
  assert.equal(workbuddy.accounts[2].status, 'error')
  assert.equal(JSON.stringify(workbuddy).includes('Real Person'), false)
  assert.equal(JSON.stringify(workbuddy).includes('real-uid'), false)

  assert.equal(canonicalGatewayProvider('ANTHROPIC'), 'claude')
  assert.equal(canonicalGatewayProvider('gemini'), null)
  assert.deepEqual(Object.keys(GATEWAY_PROVIDER_ADAPTERS).sort(), ['antigravity', 'claude', 'codex', 'kimi', 'workbuddy', 'xai'])
  for (const headers of [ANTIGRAVITY_REQUEST_HEADERS, CLAUDE_REQUEST_HEADERS, CODEX_REQUEST_HEADERS, KIMI_REQUEST_HEADERS, XAI_REQUEST_HEADERS]) assert.ok(Object.values(headers).includes('Bearer $TOKEN$'))
  assert.equal(ANTIGRAVITY_QUOTA_URLS.length, 3)
  assert.equal(GATEWAY_PROVIDER_ADAPTERS.antigravity.requests({ projectId: 'fake-project-id' }).length, 3)
  const adapterSource = readFileSync(new URL('../lib/gateway-quota-adapters.js', import.meta.url), 'utf8')
  assert.equal(adapterSource.includes('rate-limit-reset-credits/consume'), false)
  assert.equal(adapterSource.includes('chat/completions'), false)
  console.log('[ok] gateway quota pure parsers/registry/negative boundaries 通过')
}

// 21) Gateway quota host/security contract (issue #87): dynamic import only, all
// transport cases use injected fetch stubs; no real network, files, or secrets.
{
  const gateway = await import('../lib/gateway-quotas.js')
  const {
    normalizeGatewaySource,
    validateGatewaySource,
    gatewaySourceFingerprint,
    discoverGatewayAccounts,
    maskAccountLabel,
    emptyGatewayQuota,
    queryGatewayQuota,
    GATEWAY_MANAGEMENT_PATHS,
  } = gateway
  const strictAuth = gateway.__test?.strictAuth
  const isLoopbackHost = gateway.isLoopbackHost ?? gateway.__test?.isLoopback
  const requestedExportAliases = {
    normalizeGatewayBaseUrl: gateway.normalizeGatewayBaseUrl ? 'native' : 'normalizeGatewaySource',
    isLoopbackHost: gateway.isLoopbackHost ? 'native' : '__test.isLoopback',
    assertGatewayTransportAllowed: gateway.assertGatewayTransportAllowed ? 'native' : 'validateGatewaySource',
    cpaManagementFetch: gateway.cpaManagementFetch ? 'native' : 'queryGatewayQuota fetch path',
    pickAuthFileMetadata: gateway.pickAuthFileMetadata ? 'native' : '__test.strictAuth',
    discoverAccounts: gateway.discoverAccounts ? 'native' : 'discoverGatewayAccounts',
    maskLabel: gateway.maskLabel ? 'native' : 'maskAccountLabel',
    emptyGatewayQuotaSource: gateway.emptyGatewayQuotaSource ? 'native' : 'emptyGatewayQuota',
  }
  const missingDocumentedAliases = Object.entries(requestedExportAliases)
    .filter(([name, alias]) => alias !== 'native')
    .map(([name, alias]) => `${name} -> ${alias}`)
  if (missingDocumentedAliases.length > 0) {
    console.warn('[blocker] gateway host exports use current documented implementation aliases:', missingDocumentedAliases.join(', '))
  }

  assert.equal(typeof normalizeGatewaySource, 'function', 'normalizeGatewayBaseUrl contract has current normalizeGatewaySource alias')
  assert.equal(typeof validateGatewaySource, 'function', 'assertGatewayTransportAllowed contract has current validateGatewaySource alias')
  assert.equal(typeof gatewaySourceFingerprint, 'function')
  assert.equal(typeof discoverGatewayAccounts, 'function', 'discoverAccounts contract has current discoverGatewayAccounts alias')
  assert.equal(typeof maskAccountLabel, 'function', 'maskLabel contract has current maskAccountLabel alias')
  assert.equal(typeof emptyGatewayQuota, 'function', 'emptyGatewayQuotaSource contract has current emptyGatewayQuota alias')
  assert.equal(typeof queryGatewayQuota, 'function')
  assert.ok(strictAuth, 'pickAuthFileMetadata contract exposes current strict auth picker for host tests')

  const normalized = normalizeGatewaySource({
    id: 'claude-87',
    type: 'cliproxyapi',
    label: 'Local CPA',
    baseURL: 'http://127.0.0.1:8317///',
    includeProviders: ['ANTHROPIC', 'claude', 'gemini'],
    allowedHosts: ['Example.COM:8317', 'example.com:8317', ''],
    refreshMinutes: 9999,
    allowInsecureHttp: false,
  })
  assert.equal(normalized.baseURL, 'http://127.0.0.1:8317')
  assert.deepEqual(normalized.includeProviders, ['claude'], 'provider aliases canonicalize and unsupported providers drop')
  assert.deepEqual(normalized.allowedHosts, ['example.com:8317'])
  assert.equal(normalized.refreshMinutes, 1440)
  assert.equal(normalized.allowInsecureHttp, false)

  assert.equal(isLoopbackHost('localhost'), true, 'localhost is loopback')
  assert.equal(isLoopbackHost('127.42.0.9'), true, '127/8 is loopback')
  assert.equal(isLoopbackHost('[::1]'), true, '[::1] is loopback')
  assert.equal(isLoopbackHost('::ffff:127.0.0.1'), false, 'IPv4-mapped loopback is not accepted by literal policy')
  assert.equal(isLoopbackHost('127.0.0.256'), false, 'invalid 127/8 address is not loopback')

  const remoteHttps = {
    id: 'remote-87', type: 'cliproxyapi', label: 'Remote CPA',
    baseURL: 'https://CPA.Example.test/', allowedHosts: ['cpa.example.test'],
  }
  assert.equal(validateGatewaySource(remoteHttps).baseURL, 'https://cpa.example.test')
  assert.throws(
    () => validateGatewaySource({ ...remoteHttps, baseURL: 'http://cpa.example.test' }),
    error => error?.code === 'SOURCE_BLOCKED_BY_POLICY',
    'non-loopback HTTP is blocked unless explicitly enabled',
  )
  assert.equal(validateGatewaySource({ ...remoteHttps, baseURL: 'http://cpa.example.test', allowInsecureHttp: true }).allowInsecureHttp, true)
  assert.throws(
    () => validateGatewaySource({ ...remoteHttps, allowedHosts: ['other.example.test'] }),
    error => error?.code === 'SOURCE_BLOCKED_BY_POLICY',
    'remote host must exactly match the allowlist',
  )
  for (const badBaseURL of [
    'https://user:pass@cpa.example.test',
    'https://cpa.example.test/path',
    'https://cpa.example.test/?query=1',
    'https://cpa.example.test/#fragment',
    'file:///tmp/cpa',
  ]) {
    assert.throws(
      () => validateGatewaySource({ ...remoteHttps, baseURL: badBaseURL }),
      error => error?.code === 'SOURCE_BLOCKED_BY_POLICY',
      `invalid gateway origin rejected: ${badBaseURL}`,
    )
  }

  const fingerprintSource = {
    id: 'fingerprint-87', type: 'cliproxyapi', label: 'CPA',
    baseURL: 'https://cpa.example.test', allowedHosts: ['cpa.example.test'],
    includeProviders: ['claude'], allowInsecureHttp: false,
  }
  const fingerprintA = gatewaySourceFingerprint(fingerprintSource, false)
  const fingerprintB = gatewaySourceFingerprint({ ...fingerprintSource, baseURL: 'https://cpa.example.test/' }, false)
  const fingerprintKey = gatewaySourceFingerprint(fingerprintSource, true)
  ...
  const piiProject = 'PROJECT_METADATA_SENTINEL_87'
  const piiCookie = 'COOKIE_METADATA_SENTINEL_87'
  const strict = strictAuth({
    auth_index: 'claude-auth-87', provider: 'anthropic', email: piiEmail,
    project_id: piiProject, token: piiToken, cookie: piiCookie,
    id_token: 'ID_TOKEN_METADATA_SENTINEL_87', metadata: { secret: 'nested' },
  })
  assert.equal(strict.authIndex, 'claude-auth-87')
  assert.equal(strict.provider, 'anthropic')
  assert.equal(strict.email, piiEmail)
  assert.equal('token' in strict, false, 'strict auth picker drops token metadata')
  assert.equal('cookie' in strict, false, 'strict auth picker drops cookie metadata')
  assert.equal('id_token' in strict, false, 'strict auth picker drops ID-token claims')
  assert.equal('metadata' in strict, false, 'strict auth picker drops nested metadata')

  const discovered = discoverGatewayAccounts({ auth_files: [
    { auth_index: 'claude-auth-87', provider: 'anthropic', email: piiEmail, project_id: piiProject, token: piiToken },
    { auth_index: 'unsupported-87', provider: 'gemini', email: 'unsupported@example.test' },
    { provider: 'claude', email: 'missing-index@example.test', token: piiToken },
  ] }, ['claude'])
  assert.equal(discovered.accounts.length, 1)
  assert.equal(discovered.accounts[0].provider, 'claude')
  assert.deepEqual(discovered.unsupportedProviders, ['gemini'])
  assert.equal(maskAccountLabel({ email: piiEmail }), 's***@example.test', 'email label exposes only first local character and domain')
  assert.equal(maskAccountLabel({ label: 'Private account' }), 'Pr***')

  const empty = emptyGatewayQuota({ id: 'empty-87', label: 'Empty CPA', enabled: false })
  assert.equal(empty.status, 'off')
  assert.deepEqual(empty.accounts, [])
  assert.deepEqual(empty.unsupportedProviders, [])
  assert.equal(empty.keyConfigured, false)
  assert.equal(JSON.stringify(empty).includes(piiToken), false)

  const managementKey = 'MANAGEMENT_KEY_SENTINEL_87'
  const credentialContext = {
    get(name) {
      assert.equal(name, 'credentials')
      return {
        async resolve(ref) {
          assert.ok(ref, 'query resolves the source credential reference')
          return { value: managementKey, source: 'store' }
        },
      }
    },
  }
  const claudeSource = {
    id: 'claude-87', type: 'cliproxyapi', label: 'Claude Local',
    baseURL: 'http://127.0.0.1:8317', includeProviders: ['claude'],
    allowedHosts: [], allowInsecureHttp: false,
  }
  const successCalls = []
  ...
  const successFetch = async (url, init = {}) => {
    ...
    successCalls.push({ url, init, body })
    assert.equal(init.redirect, 'manual', 'management requests never follow redirects')
    assert.equal(init.headers['X-Management-Key'], managementKey, 'management key is transiently sent only in request headers')
    const path = new URL(url).pathname
    if (path === GATEWAY_MANAGEMENT_PATHS.authFiles) {
      assert.equal(body, null)
      return jsonResponse({ auth_files: [{
        auth_index: 'claude-auth-87', provider: 'anthropic', email: piiEmail,
        project_id: piiProject, token: piiToken, cookie: piiCookie,
      }] }, 200, { 'x-cpa-version': 'cpa-test-87' })
    }
    if (path === GATEWAY_MANAGEMENT_PATHS.apiCall) {
      assert.equal(body.auth_index, 'claude-auth-87')
      assert.equal(body.method, 'GET')
      assert.equal(body.url, 'https://api.anthropic.com/api/oauth/usage')
      assert.equal(body.header.Authorization, 'Bearer $TOKEN$', 'CPA receives the fixed TOKEN placeholder for substitution')
      assert.equal(JSON.stringify(body).includes(piiToken), false, 'raw auth token never enters api-call body')
      return jsonResponse({ status_code: 200, body: JSON.stringify({ five_hour: { utilization: 37 } }) })
    }
    throw new Error(`unexpected mocked gateway path: ${path}`)
  }
  const success = await queryGatewayQuota(credentialContext, claudeSource, { fetchImpl: successFetch, now: () => Date.parse('2026-09-02T00:00:00.000Z') })
  assert.equal(success.status, 'ok')
  assert.equal(success.keyConfigured, true)
  assert.equal(success.keySource, 'store')
  assert.equal(success.serverVersion, 'cpa-test-87', 'X-CPA-VERSION is retained as non-sensitive state')
  assert.equal(success.accounts.length, 1)
  assert.equal(success.accounts[0].provider, 'claude')
  assert.equal(success.accounts[0].label, 's***@example.test')
  assert.equal(success.accounts[0].windows[0].percent, 37)
  const publicSuccess = JSON.stringify(success)
  for (const sentinel of [piiEmail, piiToken, piiProject, piiCookie, managementKey]) {
    assert.equal(publicSuccess.includes(sentinel), false, `returned state excludes sentinel: ${sentinel}`)
  }
  assert.equal('authIndex' in success.accounts[0], false)
  assert.equal('email' in success.accounts[0], false)
  assert.equal('projectId' in success.accounts[0], false)
  ...
  const authFailure = await queryGatewayQuota(credentialContext, claudeSource, { fetchImpl: authFailureFetch })
  assert.equal(authFailure.status, 'error')
  assert.match(authFailure.message, /CPA_AUTH_FAILED/)
  assert.equal(authFailureCalls.length, 1, '401 is not retried')
  assert.equal(JSON.stringify(authFailure).includes(managementKey), false)
  ...
  const redirectResult = await queryGatewayQuota(credentialContext, claudeSource, { fetchImpl: redirectFetch })
  assert.equal(redirectResult.status, 'error')
  assert.match(redirectResult.message, /CPA_REDIRECT_REFUSED|SOURCE_BLOCKED_BY_POLICY/)
  assert.equal(redirectCalls.length, 1, 'redirect is refused instead of followed')

  const malformedFetch = async () => jsonResponse({ malformed: true })
  const malformedResult = await queryGatewayQuota(credentialContext, claudeSource, { fetchImpl: malformedFetch })
  assert.equal(malformedResult.status, 'error', 'malformed auth-files payload becomes a source error')
  assert.match(malformedResult.message, /CPA_RESPONSE_INVALID/)
  assert.equal(JSON.stringify(malformedResult).includes('malformed'), false, 'malformed response body is not exposed')

  const workSource = {
    id: 'workbuddy-87', type: 'cliproxyapi', label: 'WorkBuddy Local',
    baseURL: 'http://127.0.0.1:8317', includeProviders: ['workbuddy'],
  }
  const workFetch = async (url, init = {}) => {
    assert.equal(init.redirect, 'manual')
    const path = new URL(url).pathname
    if (path === GATEWAY_MANAGEMENT_PATHS.authFiles) {
      return jsonResponse({ auth_files: [{ auth_index: 'wb-known-87', provider: 'workbuddy', email: 'known@example.test' }] })
    }
    if (path === GATEWAY_MANAGEMENT_PATHS.workBuddyCredits) {
      return jsonResponse({ accounts: [{
        auth_index: 'wb-extra-87', nickname: 'WorkBuddy Person', uid: 'WORKBUDDY_UID_SENTINEL_87',
        credits: { total_remain: 380, total_used: 120, total_size: 500, packages: [] },
      }] })
    }
    throw new Error(`unexpected mocked WorkBuddy path: ${path}`)
  }
  const workResult = await queryGatewayQuota(credentialContext, workSource, { fetchImpl: workFetch })
  assert.equal(workResult.status, 'ok')
  assert.equal(workResult.accounts.length, 1, 'unmatched WorkBuddy row is retained as an independent account')
  assert.equal(workResult.accounts[0].provider, 'workbuddy')
  assert.equal(workResult.accounts[0].status, 'ok')

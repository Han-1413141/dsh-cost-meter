...
- **本会话费用**:在输入区下方(默认)或会话标题栏显示当前会话的累计费用与 token 用量;位置可在设置中切换或关闭。
- **当日费用**:侧边栏底部实时显示「今日 ¥x.xx」,悬停可见今日调用次数、token 用量、本月与累计费用。
- **设置 → 费用** 独立页面:
  - 汇总卡片:今日 / 本月 / 累计费用(含调用次数与 token);
  - 今日会话明细:每个会话的调用次数、token 与费用;
  - 历史记录:按天汇总(日期、调用、输入/输出 token、费用);

...
| 峰谷计价时段显示 | 设置页 / 预算 / 今日费用 | 显示 UTC 峰时段 01:00–04:00、06:00–10:00 与当前档位;预算与今日费用区域显示峰时/平价时段条(当前/下一时段与倒计时),可单独开关 |
...
- 峰谷计价时段显示 UTC 峰时段 01:00–04:00、06:00–10:00 与当前档位;预算框与今日费用区域显示固定左橙右蓝的峰时/平价时段条,竖条标记当前时段,标签显示当前时段与倒计时(30 秒刷新);可在设置中单独关闭,rail 窄栏不显示;

**峰时/平价时段条**:

| 中文侧边栏预算框 |
|---|
| ![中文侧边栏预算框](docs/screenshots/peak-period-strip-zh.png) |

- 时段条遵循 `peakNotice` / `peakEnabled` / `peakEffectiveAt` / `peakWindows` 门控,按 UTC 峰时窗口显示;
- 设置 → 费用 → 峰谷计价 下可单独开关「峰时高价时段显著提示」;
- 固定左橙右蓝,竖条标记当前时段;峰时橙色文字、平价蓝色文字,不显示价格。
...
- **纯峰谷两档计价**(2026-08 起官方方案):峰时段(01:00–04:00、06:00–10:00 UTC)按峰时价,其余按谷时价(谷时价 = 峰时价的一半);基础档与谷时档同价,未启用峰谷时按谷时价计;设置页实时显示当前档位(峰时段/谷时段);预算与今日费用区域显示峰时/平价时段条(当前/下一时段与倒计时);

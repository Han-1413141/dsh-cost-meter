...
| 峰谷计价 | 设置页 / 预算 / 今日费用 | DeepSeek 官方峰谷方案,带生效时间门控与当前档位状态;峰时高价时段在预算与今日费用区域显示显著提示 |
...
**PowerShell 一键脚本**(复制整行粘贴回车;自动补齐 pnpm、自动探测 git,无需克隆仓库;安装链**固定到发布 tag `v1.3.1`**,建议先下载审阅再运行):
...
- **峰谷计价按时点门控**:`peakEffectiveAt`(默认 2026-08-16 16:00 UTC)之前一律按基础价格;之后峰时段(01:00–04:00、06:00–10:00 UTC)按峰时价、其余按谷时价;设置页实时显示当前档位(未生效/峰时段/谷时段);当前处于峰时段时,预算/今日费用区域会显示一行显著提示;

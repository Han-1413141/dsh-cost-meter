...
- **新增:「含 Plan 总额」全局开关**(设置 → 显示设置):关闭 = 真金白银 API 口径;开启 = 含 Plan 等值的旧总额;覆盖卡片/徽章/预算/表格/会话徽章全部金额展示。
- **新增:路由调用归类**(provider 空/'deepseek' + 第三方目录模型 → go);**modlens-zen 存量归 Go、增量归 api**;**无明细残差归 API**。

...
## v1.7.22(2026-09-13)—— 搜索后会话恢复与版本化历史导入
...
- **旧会话修复工具**：新增 `dsh-cost-meter-repair-sessions`，默认只读检查；显式 `--write` 时取得宿主内核会话锁，保留原样备份后，只给本插件合法事件补 `ignorable: true`。保留未改动压缩帧；占用、截断、异常或超出预算的日志拒绝写入。详见 [恢复说明](session-history-recovery.md)。
...
## v1.7.21(2026-09-11)—— 会话与搜索计费、额度切换
...
## v1.7.20(2026-09-10)—— 简化侧栏回归修复与布局优化

- 修复 #121：保存失败或等待时也可关闭首次提示，浏览器记住关闭状态。
- 修复 #122：恢复简化宽栏无预算时的峰谷条，避免重复渲染。
- 完成 #123：稳定悬停分隔线、余额右对齐和充值图标、额度留白压缩、今日与预算合并。
- 双时区全量回归及隔离 DSH 0.1.5-rc.1 页面验证通过；见[简化模式说明](sidebar-simple.md)。

## v1.7.19(2026-09-10)—— 可选简化侧栏

- 解决 #118 的多功能侧栏高度占用：新增简化分区布局和面板高度上限，保留关键金额、额度与详情。
- 更新首次提示选择，配置持久化；设置中随时切换，关闭后恢复原布局。
- 双时区回归及实际 DSH 页面验证通过，覆盖中英文、深色窄栏、键盘滚动与设置保存。
- [使用说明与合成数据截图](sidebar-simple.md)。

## v1.7.18(2026-09-10)—— DeepSeek 9 月新价与历史计价

- 更新 Flash 双币种新价，并为 Pro 保留独立的 9 月 14 日切换边界。
- 旧账单保留历史价，完整日志支持升级修正；自定义价格与第三方渠道保持用户配置。
- 修复人民币目录及旧配置加载的币种混用，双时区完整回归通过。
- [价格来源、人民币/美元对照与升级边界](deepseek-pricing-2026-09.md)。

## v1.7.17(2026-09-09)—— SCNet 外部快照与项目审查修复

- 整合 PR #114：有效外部控制台快照优先显示，保留小数和真实采集时间，不混入本地账本。
- 补齐时间语义、24 小时/周期有效期、16 KiB 有界异步读取和无效快照回退。
- 增加双语接入文档、实际服务回归及中英文来源标记。
- 修复 SCNet 跨渠道误计、自定义余额空白误判、响应体限额与请求取消、混合凭据模板脱敏。
- 整合 PR #117：额度行仅使用卡片统一悬停提示，避免浏览器原生提示重叠，保留完整额度与倒计时信息。
- 完成双时区回归、三版宿主安装包加载（含 `0.1.5-alpha.1`），以及当前 alpha.2 本地链接的费用页面验证；补入新实验版的兼容声明与 peer 范围，详见[审查记录](project-audit-v1.7.17.md)。

## v1.7.16(2026-09-08)—— 侧栏布局与额度悬停提示

- 合并 PR #112 的紧凑单行布局，以及 PR #110 的窗口重置倒计时。
- 补齐双语、到期与无效日期处理、剩余方向提示、网关账号前缀及悬停/聚焦更新。
- 构建支持从其他工作目录调用，移除个人固定依赖路径。

## v1.7.15(2026-09-08)—— 千问渠道隔离与默认峰谷价修复

- 千问 Credits 只统计订阅 provider，同名模型经 `qianwen` 按量渠道调用不再挤占估算额度；显式 API 分类继续生效。
- DeepSeek 默认回退保留峰谷和历史价，修复峰时套用谷价造成的约半价路径，美元、人民币及官方同步均覆盖。
- 升级时仅补齐符合旧内置值的默认价，并按完整日志逐事件修复历史；缺失日志保留原金额，主动清空价格档位可持久保存。
- #109 尚缺反馈者的原始配置和调用日志，本版修复已复现的默认回退路径，原截图的实际路径待核实。
...
## v1.7.6(2026-09-01)—— 🔐 自定义余额凭据治理(issue #86)
- **安全**:请求头里的明文密钥(Bearer sk-… / X-Api-Key)此前会明文落盘 ledger.json 并随页面下发——现按值形态判定一律置空(占位符与普通头不受影响),落盘/下发/补丁三条路径同一闸门;启动时自动把存量明文迁入 DSH 凭据库并替换为稳定占位符,重启后功能不丢。
- **防护**:凭据外带判定从 {{VAR}} 占位符扩展到明文 key——白名单(allowedHosts)外的主机会被直接拒绝,明文不再绕过校验。
- **UI**:条目面板新增「凭据白名单」输入框(此前有字段无入口)与逐占位符的 write-only「凭据输入」框(密钥直接存凭据库,不落盘不回显);请求头上方标注 {{VAR}} 命名规则(<ROUTE>_API_KEY,与模型页同名即共用同一把密钥)。

## v1.7.5(2026-09-01)—— 🧮 GLM-5.3 补价 + GLM-5.3-Flash 新增(issue #85)
- **修复**:智谱官方已公布 GLM-5.3 定价(国内 ¥8/¥28/缓存¥2)，扩展目录按 OpenCode Go 核价收录 .40/$0.26/$4.40——此前的 unpriced 标记随之退役。
- **新增 GLM-5.3-Flash**(Ox Alpha 普惠版，2026-08-26 发布）：双目录收录 $0.15/$0.03/$0.50，大小写归一命中，可在拓展价格表挂载编辑。
- **消歧**：设置页新增说明——「从官方文档同步价格」仅更新 DeepSeek 官方价，第三方目录价随插件版本维护(可在拓展价格表编辑或经模型名匹配手动指定)。

## v1.7.4(2026-09-01)—— 📦 依赖升级(#82/#83/#84)
- **升级**:官方运行时包 dsh-credentials / dsh-home-paths → 0.1.0-rc.8,zod → 4.5.1;esbuild → 0.28.2。本地冻结安装下全量测试双时区绿、实机 dsh web 启动冒烟通过。
- **修正锁版门窟断言**:不再硬编码具体版本号(此前任何依赖升级都会误报红，本次三个 Dependabot PR 全红的根因);改锁精确格式，并新增 workspace 排除表与锁版的一致性断言。

## v1.7.3(2026-09-01)—— ⚡ 中转链路缓存未上报显式标注(issue #65 讨论)
- **改进**:经中转/代理调用时缓存字段可能被剥离(或中转导致缓存真实未命中),插件端命中率恒 0% 且金额按全未命中价计(上界)——此前无从区分。现按模型统计面板对「多次长上下文却恒零命中」的模型显式标注「未上报?」并附说明;真零命中(单轮/短上下文)不会被误标。
- **边界说明**:插件无法恢复被剥离的数据或修复中转导致的真实未命中——请求由宿主/中转组装,插件仅为计量方;本改进是消除误导性显示。

## v1.7.2(2026-09-01)—— 💱 对账警告币种符号修复(issue #81)
- **修复**:CNY 账户触发余额对账警告时,同一句话里两个金额的币种符号不一致(本地费用 $ 、官方变动 ¥)。现两侧统一按账户币种显示:CNY 账户的本地费用按展示汇率折算为 ¥ 并附 ≈$ 参考,与变动侧对称。

## v1.7.1(2026-08-31)—— 🚨 紧急修复:v1.7.0 导致 dsh 无法启动
- **修复**:v1.7.0 给自定义余额刷新接口新增的参数声明写错了 codec 形态,导致完整插件树加载失败、dsh web 无法启动。现已按 typert 契约改为标准形态并允许旧调用无参传递;并新增「宿主级 manifest 校验」回归测试,此类错误今后在测试期即暴露。

## v1.7.0(2026-08-31)—— 💳 自定义 Provider 余额多配置(issue #79)
- **新增自定义 Provider 余额支持多条并行(最多 8 条)**——设置页可逐条添加/删除,每条独立的名称、币种、显示位置、刷新间隔、请求与解析规则;侧边栏按条目逐卡显示余额,点击逐条刷新。
- **无缝升级**——升级前的单条配置自动迁移为第 1 条,无需任何操作;每条独立缓存与自动刷新,失败的条目单独显示错误不影响其它条目。

## v1.6.13(2026-08-31)—— 🐋 千问 Qwen Token Plan 接入(issue #78)
- **新增:千问 AI 平台 Token Plan 个人版额度显示**——设置 → Coding Plan 新增「千问 Qwen Token Plan」卡片,展示月度 Credits 用量百分比与已用/总额,可上侧边栏。
- **本地 Credits 计量**:千问未提供 API-Key 化的额度查询端点(额度仅控制台可见),与 SCNet 同模式按官方 Credits 抵扣率由本地账本估算——无需任何凭据,不发网络请求。内置个人版 10 个模型的抵扣率,官方调价或表外模型可在设置中逐模型覆盖;月度 Credits 额度与订阅起始日均可配置。
- 千问渠道调用默认按 Plan 类计费(等值不动真金白银),与其它订阅渠道口径一致。

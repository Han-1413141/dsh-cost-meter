 ...
 */
export function backfillLegacyLedger(ledger, sessionsRoot) {
  ...
  for (const [date, day] of Object.entries(ledger.days ?? {})) {
    ...
    if (dayIsEmpty) {
      ...
      const replayedCalls = Object.values(aggregate).reduce((sum, b) => sum + (b.calls ?? 0), 0)
      if (replayedCalls < (day.calls ?? 0)) {
        ...
        aggregate['deepseek:legacy'] = {
          ...
          calls: (day.calls ?? 0) - replayedCalls,
    ...
    // 会话级:按会话 id + 日期定向填补空条目。
    for (const session of day?.sessions ?? []) {
      if ((session?.calls ?? 0) <= 0) continue
      if (Object.keys(session?.byProviderModel ?? {}).length > 0) continue

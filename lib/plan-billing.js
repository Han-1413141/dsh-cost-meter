 ...
 * 尾部——周/月估算因此系统性偏低。
 */
export function aggregateUsageSince(days, hourBuckets, provider, startMs, nowMs, planBilling, enabledPlans) {
...
export function detectPlanProviders(days) {
  ...
  for (const day of Object.values(days ?? {})) {
    ...
    for (const [key, entry] of Object.entries(day?.byProviderModel ?? {})) {
      ...
      cur.tokens += (entry?.input ?? 0) + (entry?.output ?? 0) + (entry?.cacheRead ?? 0) + (entry?.cacheWrite ?? 0) + (entry?.reasoning ?? 0)
...
export function buildPlanStats({ days, hourBuckets, samples, codingPlans, goQuota, config, nowMs }) {
  ...
  const collect = (providerId, status, windowsRaw) => {
    ...
    for (const [name, win] of Object.entries(windowsRaw)) {
      ...
      const local = aggregateUsageSince(days, hourBuckets, providerId, start, now, planBilling, enabledPlans)

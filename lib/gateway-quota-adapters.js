 ...
 *
 * 设计契约(见 gateway-design.md §4 / protocol-evidence.md §2–§8):
 ...
 *   各家 payload 的数字语义不同(小数份额 / 0–100 百分比 / 绝对量),必须按
 *   protocol-evidence.md 逐家转换,严禁共用 coding-plans 的 normalizePercent——
 *   它会猜 0–1 还是百分比,与证据冲突(issue #87 §8.5)。
 * - 付费档 xAI 探活 probe(向对话补全端点 POST 一条真实消息)**故意不实现**:
 *   它会消耗 token,额度表(meter)绝不允许产生任何计费副作用(§5.4/§8.4)。
 */
...
// ---------------------------------------------------------------------------
// 注册表(adapter 契约见 gateway-design.md §4)
// ---------------------------------------------------------------------------

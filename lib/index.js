...
import { Ledger, applyConfigPatch, localDayKey, pickBalanceInfo, reconcileBalanceDelta, zeroDay, splitLedgerApiCost, repairLedgerPricing, dedupeWrapperProviderDays, stripSecrets, stripSecretPatch, secretRefOf, readSecret, writeSecret, SECRET_TARGETS } from './store.js'
...
function makeCostUsageProjection(ledger) {
  ...
  return {
    ...
    apply(state, event) {
      ...
      // 且因 provider 不同绕过 (turn, step) 键去重,投影会翻倍(issue #70)。与账本
      // 钩子一致,包装层 provider 状态下的 usage 事件跳过,只统计上游真实流。
      if (isWrapperProviderId(state.provider)) return state
      const buckets = {
...
export function apply(ctx) {
  ...
  // 包装层跳过(issue #70):现行 modlens(modlens-<upstream> id 方案)在监听器
  // 体内(瀑布派发期)急切发起上游调用,逃逸 ALS 深度标记,上游与包装层两份
  // usage 都到达这里——账本对 provider 为包装层 id 的事件直接跳过,只记上游
  // 真实流;历史翻倍由启动期 modlens-wrapper-dedup-v1 清洗兜底。
  ctx.on('llm/stream', createLlmStreamBilling({
    account: (usage, model, sessionId, atMs, provider) => {
      if (isWrapperProviderId(provider)) return
      ledger.account({
        input: usage.inputTokens ?? 0,
        output: usage.outputTokens ?? 0,
        cacheRead: usage.cacheReadTokens ?? 0,

...
/** 组装对客户端的完整账本快照。 */
function buildState(ledger, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}, customBalance = emptyCustomBalance()) {
  const now = Date.now()
...
function createService(ctx, ledger) {
  ...
  const build = async (forceBalance = false) => {
    ...
    return buildState(ledger, balanceCache.value, goQuotaCache.value, mergedCodingPlans(), customBalanceCache.value)
  }
  ...
  const service = {
    ...
    async refreshBalance() {
      ...
      return {
        ...
        state: buildState(ledger, value, goQuotaCache.value, mergedCodingPlans(), customBalanceCache.value),
      }
    },
    ...
    async refreshCustomBalance() {
      ...
      return {
        ...
        state: buildState(ledger, balanceCache.value, goQuotaCache.value, mergedCodingPlans(), value),
      }
    },
    ...
    async refreshGoQuota() {
      ...
      return {
        ...
        state: buildState(ledger, balanceCache.value, value, mergedCodingPlans(), customBalanceCache.value),
      }
    },
    ...
    async refreshCodingPlan(provider) {
      ...
      return {
        ...
        state: buildState(ledger, balanceCache.value, goQuotaCache.value, mergedCodingPlans(), customBalanceCache.value),
      }
    },

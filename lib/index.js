...
import { Ledger, applyConfigPatch, localDayKey, pickBalanceInfo, reconcileBalanceDelta, zeroDay, splitLedgerApiCost } from './store.js'
...
import { OFFICIAL_PRICING_URL, OFFICIAL_PRICING_URL_ZH, normalizePrice, parsePricingHtml, costOf, priceEntryFor, providerPriceEntryFor, buildPriceCatalog, usdFromCost } from './pricing.js'
...
import { recordSamples, buildPlanStats, canonicalWindowKey, periodStartOf, aggregateUsageSince, enabledPlanSetOf, pruneRecentCalls } from './plan-billing.js'
...
function buildState(ledger, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}, customBalance = emptyCustomBalance(), reconcile = { ok: true, message: '' }) {
  ...
  ledger.planRecentCalls = pruneRecentCalls(ledger.planRecentCalls, now)
  const planStats = buildPlanStats({
    days: ledger.days ?? {},
    recentCalls: ledger.planRecentCalls,
  ...
  // 真金白银口径(issue #64):只计 API 渠道(apiCost),Plan 订阅额度消耗不动预算。
  const apiCostOf = agg => Number(agg?.apiCost ?? agg?.cost ?? 0)
  const budget = ledger.config?.budget ?? {}
  let budgetUsed
  if (budget.period === 'day') budgetUsed = apiCostOf(ledger.today())
  else if (budget.period === 'all') budgetUsed = apiCostOf(ledger.sumDays(undefined))
    ...
    budgetUsed = start === null ? 0 : apiCostOf(ledger.sumRange(start, end))
  } else budgetUsed = apiCostOf(ledger.sumDays(monthKey))
  const state = {
    today: ledger.today(),

  ...
  const service = {
    ...
    async refreshGatewayQuota(sourceId = null) {
      ...
      if (id !== null && !sources.some(source => source.id === id)) {
        return { ok: false, message: 'Unknown gateway quota source' }
      }
      const selected = id === null ? sources : sources.filter(source => source.id === id)
      if (selected.length === 0) return { ok: false, message: 'No gateway quota source is configured' }
      ...
      return {
        ok,
        message: ok ? 'Gateway quota refreshed' : selectedValues.map(value => value.message).filter(Boolean).join('; ') || 'Gateway quota refresh failed',

...
const SERVER_MESSAGES = {
  zh: {
    ...
    goQuotaKeyMissing: '未找到 OpenCode Go API Key(请在设置中填写,或设置 OPENCODE_API_KEY 环境变量,或先运行 opencode 登录)',
    goQuotaHttp: 'OpenCode Go 额度接口 HTTP {code}',
    goQuotaNoSub: 'OpenCode Go 额度接口返回 {code}:可能没有生效的 Go 订阅,或 API Key 无效',
  ...
  en: {
    ...
    goQuotaKeyMissing: 'OpenCode Go API key not found (set it in Settings, export OPENCODE_API_KEY, or run opencode login first)',
    goQuotaHttp: 'OpenCode Go quota API returned HTTP {code}',
    goQuotaNoSub: 'OpenCode Go quota API returned {code}: no active Go subscription, or the API key is invalid',
...
/** 解析 OpenCode Go API Key:显式配置 → OPENCODE_API_KEY 环境变量 → opencode auth.json。 */
function resolveGoKey(config) {
 ...
 * 凭证只发往官方域名 opencode.ai;Key 解析顺序:配置 → OPENCODE_API_KEY → opencode auth.json。
 * @param config - 插件配置(goQuota.apiKey / 消息语言)。
 * @param locale - 消息语言(zh/en)。
 */
async function queryGoQuota(config, locale) {
  const key = resolveGoKey(config)
  if (key === null) throw new Error(tmsg(locale, 'goQuotaKeyMissing'))
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw new Error(tmsg(locale, 'goQuotaNoSub', { code: String(response.status) }))
    }
...
function createService(ctx, ledger) {
  ...
  const ensureGoQuota = async (force = false) => {
    ...
    const task = queryGoQuota(ledger.config, localeOf(ledger.config)).then(result => {
  ...
  const service = {
    ...
    async refreshGoQuota() {
      ...
      return {
        ok: value.status === 'ok',
        message: value.status === 'ok' ? tmsg(locale, 'goQuotaRefreshed') : tmsg(locale, 'goQuotaQueryFailed', { message: value.message }),

 ...
 */
async function buildState(ledger, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}, customBalance = emptyCustomBalance(), reconcile = { ok: true, message: '' }, ctx = null, customBalances = []) {
  const now = Date.now()
...
function createService(ctx, ledger) {
  ...
  const build = async (forceBalance = false) => {
    ...
    return await buildState(ledger, balanceCache.value, goQuotaCache.value, await mergedCodingPlans(), customBalanceMirror(), reconcileNotice, ctx, customBalanceSnapshots())
  }
  ...
  const service = {
    ...
    async refreshBalance() {
      ...
      return {
        ...
        state: await buildState(ledger, value, goQuotaCache.value, await mergedCodingPlans(), customBalanceMirror(), reconcileNotice, ctx, customBalanceSnapshots()),
      }
    },
    ...
    async refreshCustomBalance(index = null) {
      ...
      return {
        ...
        state: await buildState(ledger, balanceCache.value, goQuotaCache.value, await mergedCodingPlans(), customBalanceMirror(), reconcileNotice, ctx, customBalanceSnapshots()),
      }
    },
    ...
    async refreshGoQuota() {
      ...
      return {
        ...
        state: await buildState(ledger, balanceCache.value, value, await mergedCodingPlans(), customBalanceMirror(), reconcileNotice, ctx, customBalanceSnapshots()),
      }
    },

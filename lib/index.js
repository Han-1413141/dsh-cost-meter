...
function makeCostUsageProjection(ledger) {
  ...
  return {
    ...
    apply(state, event) {
      ...
      const rawProvider = typeof state.provider === 'string' ? state.provider : ''
      ...
      if (prev !== null && prev.provider === effectiveProvider && prev.model === state.model
      ...
      // 超过 24 条先截断再判定,checkpoint 体积恒定。
      const fp = usageFingerprint(state.model, buckets)
      ...
      if (wrapped ? (hitPlain || hitRemapped) : hitRemapped) {
        return { ...state, recent }
      }
      recent.push({ fp, at: atMs, ...(wrapped ? { w: 1 } : {}) })
      const resolved = providerPriceEntryFor(effectiveProvider, state.model, ledger.config?.prices, {
      ...
      shiftInto(mainAgg, effectiveProvider, state.model, buckets, billed, 1)
      if (mirrorSeed) {
        if (prev !== null) shiftInto(shadowAgg, prev.provider, prev.model, prev.buckets, prev.cost, -1)
        shiftInto(shadowAgg, effectiveProvider, state.model, buckets, billed, 1)
      }
      // createdAt/seedLength/seedDeducted 必须随状态携带:usage 样本更新不能丢掉 fork 过滤基准。
      // recent 随行:包装层转发对去重的滚动窗口状态(有界 ≤25 条)。
      return { provider: state.provider, model: state.model, totals, byModel, byProviderModel, createdAt: state.createdAt, seedEndSeq: state.seedEndSeq, seedLength: state.seedLength, seedDeducted: state.seedDeducted, shadow: shadowAgg, recent, last: { key, provider: effectiveProvider, model: state.model, buckets, cost: billed } }
    },
    view: projectionView,

...
async function queryGoQuota(config, locale) {
  ...
  if (!response.ok) throw new Error(tmsg(locale, 'goQuotaHttp', { code: String(response.status) }))
...
function createService(ctx, ledger) {
  ...
  const goQuotaConfig = () => ledger.config?.goQuota ?? { display: 'both', refreshMinutes: 15, apiKey: '' }
  ...
  /** 按需刷新 OpenCode Go 额度(过期或 force);失败落 error 状态。 */

...
import { CODING_PLAN_PROVIDERS, CODING_PLAN_PROVIDER_IDS, queryCodingPlan } from './coding-plans.js'
...
const SERVER_MESSAGES = {
  zh: {
    ...
    codingPlanHttp: '{provider} 额度接口 HTTP {code}',
  ...
  en: {
    ...
    codingPlanHttp: '{provider} quota API returned HTTP {code}',
...
/** 组装对客户端的完整账本快照。 */
function buildState(ledger, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}) {
  const now = Date.now()
  ...
  const attempts = [
    ...
    { ...state, priceCatalog: undefined, codingPlans: {}, balance: emptyBalance(), goQuota: emptyGoQuota() },
  ]
...
function createService(ctx, ledger) {
  ...
  const build = async (forceBalance = false) => {
    await Promise.all([ensureBalance(forceBalance), ensureGoQuota(false), ensureCodingPlans(false)])
    return buildState(ledger, balanceCache.value, goQuotaCache.value, mergedCodingPlans())
  }
  ...
  const service = {
    ...
    async refreshBalance() {
      ...
      return {
        ...
        state: buildState(ledger, value, goQuotaCache.value, mergedCodingPlans()),
      }
    },
    ...
    async refreshGoQuota() {
      ...
      return {
        ...
        state: buildState(ledger, balanceCache.value, value, mergedCodingPlans()),
      }
    },
    ...
    async refreshCodingPlan(provider) {
      ...
      return {
        ...
        state: buildState(ledger, balanceCache.value, goQuotaCache.value, mergedCodingPlans()),
      }
    },
...
export function apply(ctx) {
  ...
  const backfillTimer = setTimeout(() => {
    try {
      const filled = backfillLegacyLedger(ledger, join(resolveDshHome(), 'sessions'))
      if (filled.days > 0 || filled.sessions > 0) {
        console.log(`[dsh-cost-meter] 历史按模型统计回填完成:${filled.days} 天 / ${filled.sessions} 个会话(扫描 ${filled.scanned} 份会话日志)`)
      }
    } catch (error) {

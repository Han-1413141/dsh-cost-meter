...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '@media (max-width:640px){.cm-cards{grid-template-columns:1fr}.cm-grid{grid-template-columns:1fr}}',
    ].join('\n')
    ...
    function SessionCost(props) {
      ...
      const detail = [
        '本会话费用(按当前价格估算)',
        '输入 ' + formatTokens(input) + ' · 输出 ' + formatTokens(usage.output ?? 0),
        '缓存命中 ' + formatTokens(usage.cacheRead ?? 0),
    ...
    function SidebarBadge(props) {
      ...
      const detail = [
        '今日费用(按官方价格精确计费)',
        '调用 ' + today.calls + ' 次 · 输入 ' + formatTokens(today.input + today.cacheRead + today.cacheWrite) + ' · 输出 ' + formatTokens(today.output),
      ...
      return el('div', { className: 'cm-scroll' },
        el('table', { className: 'cm-table' },
          el('thead', null, el('tr', null,
            ...
            el('th', { className: 'num' }, '输入 tok'), el('th', { className: 'num' }, '输出 tok'),
            el('th', { className: 'num' }, '费用'))),
          el('tbody', null, rows.map(day => el('tr', { key: day.date },
            el('td', null, day.date),
            el('td', { className: 'num' }, String(day.calls)),
            el('td', { className: 'num' }, formatTokens(day.input + day.cacheRead + day.cacheWrite)),
      ...
      return el('div', { className: 'cm-scroll' },
        el('table', { className: 'cm-table' },
          ...
          el('tbody', null, sessions.map(session => el('tr', { key: session.id },
            ...
            el('td', { className: 'num' }, formatTokens(session.input + session.cacheRead + session.cacheWrite)),
    ...
    function CostSection(props) {
      ...
      useEffect(() => {
        if (state !== null) setDraft(JSON.parse(JSON.stringify(state.config)))
      }, [state])
      ...
      const save = async () => {
        ...
        try {
          ...
          setMessage({ kind: 'err', text: '保存失败:' + (error?.message ?? String(error)) })
        } finally {
          setBusy(false)
        }
      }
      ...
      return el('div', { className: 'cm-section' },
        ...
        el('div', { className: 'cm-cards' },
          el(Card, { title: '今日费用', value: formatMoneyUsd(state.today.cost, config), sub: '调用 ' + state.today.calls + ' 次 · 输入 ' + formatTokens(state.today.input + state.today.cacheRead + state.today.cacheWrite) + ' · 输出 ' + formatTokens(state.today.output) }),
          el(Card, { title: '本月费用', value: formatMoneyUsd(state.month.cost, config), sub: '调用 ' + state.month.calls + ' 次' }),
          el(Card, { title: '累计费用', value: formatMoneyUsd(state.total.cost, config), sub: '自账本建立以来' })),
        // 今日会话
        el('div', null,
        ...
        el('div', null,
          ...
          el('p', { className: 'cm-note' }, '会话徽章显示的是「本会话」累计估算费用;当日/月度/累计为按官方价格与实际时刻(含峰谷)精确计费的账本数据。')),
        // 价格表
        el('div', null,
          el('h3', { className: 'cm-h' }, '价格表(美元 / 1M tokens)'),
          el('p', { className: 'cm-note' }, '「谷时/峰时」为峰谷计价生效后的价格;缓存写入按缓存命中价格计费(与官方规则一致)。修改后点击保存。'),
          priceCards,
          el(PriceCard, {
        ...
        el('div', null,
          ...
          el('div', { className: 'cm-buttons' },
            el('button', { className: 'cm-btn primary', onClick: save, disabled: busy || draft === null }, busy ? '保存中…' : '保存设置'),
            confirmFetch
              ? el(Fragment, null,

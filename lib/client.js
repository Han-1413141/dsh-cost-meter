...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 预算图框内容(不含外框),供单独显示与「Go+预算」合并卡片复用;compact 用于合并卡片(省略次要行)。 */
    function budgetBoxBody(state, config, t, compact = false) {
    ...
    /** OpenCode Go 额度图框内容(不含外框),与预算图框同风格;主档位可配(默认 5h),其余档位在下方一行展示;compact 用于合并卡片(省略重置行)。 */
    function goBoxBody(state, config, t, compact = false) {
      const goQuota = state.goQuota
      const mainKey = config?.goQuota?.main === 'weekly' || config?.goQuota?.main === 'monthly' ? config.goQuota.main : 'rolling'
      ...
      return {
        ...
        body: el(Fragment, null,
          ...
          compact ? null : (resets ? el('div', { className: 'cm-bbox-line' }, resets) : null)),
      }
    }
    ...
    function SidebarFooter(props) {
      ...
      if (goOk && budgetOn && wide) {
        // 同时出现:合并为一张卡片(Go 在上、预算在下,细分隔线),各自保留预警色;合并时用紧凑体压缩高度。
        const goView = goBoxBody(state, config, t, true)
        const budgetView = budgetBoxBody(state, config, t, true)
    ...
    function CostSection(props) {
      ...
      return el('div', { className: 'cm-section' },
        ...
        // OpenCode Go 订阅额度(顶部,紧随预算;按显示配置)

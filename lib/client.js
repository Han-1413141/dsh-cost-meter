    ...
    function BalanceRowContent(props) {
      ...
      if (balance.status === 'error') {
        return el(Tooltip, { label: '余额查询失败:' + (balance.message || '未知错误'), side: 'right', delayMs: 300 },
          el('div', { className: 'cm-foot' + (wide ? '' : ' cm-foot-rail') + ' cm-bal-err' },
            wide ? el(Fragment, null, '余额 ', el('span', { className: 'cm-num' }, '查询失败')) : '⚠'))
      }
      const detail = [
        'DeepSeek 开放平台账户余额',
      ...
      return el(Tooltip, { label: detail, side: 'right', delayMs: 300 },
        el('div', { className: 'cm-foot' + (wide ? '' : ' cm-foot-rail') },
          wide ? el(Fragment, null, '余额 ', el('span', { className: 'cm-num' }, formatBalanceMoney(balance.totalBalance, state.config))) : (state.config.symbol || '¥')))
    }
    ...
    function BudgetBoxContent(props) {
      ...
      if (budget.enabled === true) {
        ...
        const detail = [
          ...
          '今日 ' + formatMoneyUsd(today.cost, config) + ' · 占预算 ' + (todayPct === null ? '—' : todayPct.toFixed(1) + '%'),
          '本月 ' + formatMoneyUsd(state.month.cost, config) + ' · 累计 ' + formatMoneyUsd(state.total.cost, config),
        ].join(';')
        return el(Tooltip, { label: detail, side: 'right', delayMs: 300 },
      ...
      return el(Tooltip, { label: detail, side: 'right', delayMs: 300 },
        el('div', { className: 'cm-foot' + (wide ? '' : ' cm-foot-rail') },
          wide ? el(Fragment, null, '今日 ', el('span', { className: 'cm-num' }, formatMoneyUsd(today.cost, config))) : '¥'))
    }

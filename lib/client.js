...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GoRowContent(props) {
      const { state, wide } = props
      const goQuota = state.goQuota
      const t = makeT(resolveLocale(state.config?.locale))
      if (!goQuota || goQuota.status !== 'ok') return null
      const monthly = goQuota.monthly
      if (monthly === null || typeof monthly.percent !== 'number') return null
      const pctOf = win => (win !== null && typeof win?.percent === 'number') ? Math.round(Math.max(0, Math.min(100, win.percent))) + '%' : '—'
      const detail = [
        t('goQuotaTitle'),
        t('goWindowRolling') + ': ' + pctOf(goQuota.rolling),
        t('goWindowWeekly') + ': ' + pctOf(goQuota.weekly),
        t('goWindowMonthly') + ': ' + pctOf(monthly)
          + (typeof monthly.resetsAt === 'string' && monthly.resetsAt.length > 0 ? ' · ' + t('goResetAt', { time: new Date(monthly.resetsAt).toLocaleString() }) : ''),
        t('goQuotaFetchedAt', { time: goQuota.fetchedAt > 0 ? new Date(goQuota.fetchedAt).toLocaleTimeString() : '—' }),
      ].join('; ')
      return el(Tooltip, { label: detail, side: 'right', delayMs: 300 },
        el('div', { className: 'cm-foot' + (wide ? '' : ' cm-foot-rail') },
          wide
            ? el(Fragment, null, t('goQuotaRowLabel'), ' ', el('span', { className: 'cm-num' }, pctOf(monthly)))
            : el('span', { className: 'cm-num cm-go-rail' }, pctOf(monthly))))
    }
    ...
    function SidebarFooter(props) {
      ...
      const showGo = (config.goQuota?.display === 'sidebar' || config.goQuota?.display === 'both')
        && state.goQuota?.status === 'ok' && state.goQuota?.monthly !== null
      const showToday = config.sidebar !== false
      if (!showBalance && !showGo && !showToday) return null
      // 外壳的 footerActions 是横向 flex;这里用自建纵向堆叠保证余额在上、图框在下。
      return el('div', { ref: rootRef, className: 'cm-footer-stack' + (wide ? '' : ' rail') },
        showBalance ? el(BalanceRowContent, { state, wide }) : null,
        showGo ? el(GoRowContent, { state, wide }) : null,
        showToday ? el(BudgetBoxContent, { state, wide }) : null)
    }

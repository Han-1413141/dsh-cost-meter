    ...
    async function apply(ctx) {
      ...
      const registerSidebar = enabled => {
        ...
        slots.inject('sidebar.footer.action', () => {
          ...
          const dispose = slots.register({ name: 'sidebar.footer.action', id: 'cost-meter', order: 0, inject: injected }, SidebarBadge)
      ...
      const registerBalance = enabled => {
        ...
        slots.inject('sidebar.footer.action', () => {
          ...
          // 负 order:余额行渲染在预算图框/今日徽章之上(两者都在设置按钮上方)。
          const dispose = slots.register({ name: 'sidebar.footer.action', id: 'cost-meter-balance', order: -10, inject: injected }, SidebarBalanceRow)

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.cm-peak-notice{margin-top:6px;padding:6px 8px;border:1px solid var(--dsw-alias-state-warn-primary);border-radius:8px;background:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 12%,transparent);color:var(--dsw-alias-state-warn-primary);font-size:12px;font-weight:600;line-height:1.4}',
    ...
    function peakNoticeEl(state, config, t) {
      ...
      const now = Date.now()
      if (Number.isFinite(effectiveAt) && now < effectiveAt) return null
      ...
      return inPeak ? el('div', { className: 'cm-peak-notice' }, t('peakNotice')) : null
    }

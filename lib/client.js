    ...
    function usageCost(usage, config) {
      ...
      for (const providerKey of Object.keys(byModel)) {
        ...
        if (resolved.priced) {
          ...
          total += usdFromCostLocal(c, resolved.billingMode === 'deepseek-peak' && config.prices?.currency === 'CNY' ? 'CNY' : 'USD', config.exchangeRate)
        }
      }
      ...
      total += usdFromCostLocal(leftoverCost, config.prices?.currency === 'CNY' ? 'CNY' : 'USD', config.exchangeRate)
      return total
    }

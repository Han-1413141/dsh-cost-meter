...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.cm-bbox-pair{display:flex;flex-direction:column;gap:0;padding:4px 10px}',
      '.cm-bbox-pair .cm-bbox-section{display:flex;flex-direction:column;gap:6px;padding:6px 0}',
    ...
    const MESSAGES = {
      zh: {
        // 会话徽章
        sessionCostTitle: '本会话费用(按当前价格估算)',
        ...
        badgeNote: '会话徽章显示的是「本会话」累计估算费用;当日/月度/累计为按官方价格与实际时刻(含峰谷)精确计费的账本数据。输入/缓存/输出 token 分开统计,缓存读写按命中价计费。',
        priceTableTitle: '价格表(美元 / 1M tokens)',
        priceTableNote: '「谷时/峰时」为峰谷计价生效后的价格;缓存写入按缓存命中价格计费(与官方规则一致)。所有设置修改后自动保存。',
      ...
      en: {
        sessionCostTitle: 'Cost of this session (estimated at current prices)',
        ...
        badgeNote: 'The session badge shows the estimated accumulated cost of THIS session; today/month/all-time figures are exact ledger data billed at official prices and the actual call time (including peak/off-peak). Input/cache/output tokens are counted separately; cache reads and writes are billed at the cache-hit price.',
        priceTableTitle: 'Price table (USD / 1M tokens)',
        priceTableNote: '"Off-peak / Peak" are the prices used once peak/off-peak pricing takes effect; cache writes are billed at the cache-hit price (matching the official rule). All settings changes are auto-saved.',
    ...
    function CornerChips(props) {
      ...
      pushGo(corner.goRolling === true, goQuota?.rolling, 'goShortRolling', 'cornerGoRolling')
      pushGo(corner.goWeekly === true, goQuota?.weekly, 'goShortWeekly', 'cornerGoWeekly')
      pushGo(corner.goMonthly === true, goQuota?.monthly, 'goShortMonthly', 'cornerGoMonthly')
    ...
    /** 预算图框内容(不含外框),供单独显示与「Go+预算」合并卡片复用。 */
    ...
    /** OpenCode Go 额度图框内容(不含外框),与预算图框同风格。 */
    function goBoxBody(state, t) {
      const goQuota = state.goQuota
      const monthly = goQuota.monthly
      const pct = Math.max(0, Math.min(100, Number(monthly.percent) || 0))
      const pctOf = win => (win !== null && typeof win?.percent === 'number') ? Math.round(Math.max(0, Math.min(100, win.percent))) + '%' : '—'
      const resets = typeof monthly.resetsAt === 'string' && monthly.resetsAt.length > 0
        ? t('goResetAt', { time: new Date(monthly.resetsAt).toLocaleString() })
        : ''
      return {
        level: pct >= 100 ? 'over' : pct >= 80 ? 'warn' : 'ok',
        rail: Math.round(pct) + '%',
        body: el(Fragment, null,
          el('div', { className: 'cm-bbox-head' },
            el('span', { className: 'cm-bbox-label' }, t('goQuotaRowLabel')),
            el('span', { className: 'cm-bbox-pct cm-num' }, pct.toFixed(1) + '%')),
          el('div', { className: 'cm-bbox-bar' },
            el('div', { className: 'cm-bbox-fill', style: { width: Math.min(100, pct) + '%' } })),
          el('div', { className: 'cm-bbox-line cm-num' },
            t('goShortRolling') + ' ' + pctOf(goQuota.rolling) + ' · ' + t('goShortWeekly') + ' ' + pctOf(goQuota.weekly)),
    ...
    function GoQuotaBox(props) {
      ...
      if (goQuota.monthly === null || typeof goQuota.monthly.percent !== 'number') return null
      const view = goBoxBody(state, t)
      const detail = [
        t('goQuotaTitle'),
    ...
    function SidebarFooter(props) {
      ...
      const goOk = (config.goQuota?.display === 'sidebar' || config.goQuota?.display === 'both')
        && state.goQuota?.status === 'ok' && state.goQuota?.monthly !== null
      ...
      if (goOk && budgetOn && wide) {
        // 同时出现:合并为一张卡片(Go 在上、预算在下,细分隔线),各自保留预警色。
        const goView = goBoxBody(state, t)
    ...
    function GoQuotaPanel(props) {
      ...
      const body = goQuota.status === 'ok'
        ? el('div', { className: 'cm-go-list' },
          windowRow('goWindowRolling', goQuota.rolling),
          windowRow('goWindowWeekly', goQuota.weekly),
          windowRow('goWindowMonthly', goQuota.monthly),
      ...
      return el('div', { className: 'cm-budget' },
        el('div', { className: 'cm-budget-head' },
          ...
          el('button', { className: 'cm-btn small', onClick: doRefresh, disabled: busy }, busy ? t('refreshing') : t('refreshGoQuota'))),
        body,
        msg !== null ? el('div', { className: 'cm-msg ' + msg.kind }, msg.text) : null)
    }
    ...
    function CostSection(props) {
      ...
      return el('div', { className: 'cm-section' },
        ...
        (config.balance?.display === 'settings' || config.balance?.display === 'both')
          ...
          : null,
        // OpenCode Go 订阅额度(按显示配置)
        (config.goQuota?.display === 'settings' || config.goQuota?.display === 'both')
          ? el(GoQuotaPanel, { state, api, t })
          : null,
        // 汇总卡片
    ...
    async function apply(ctx) {
      ...
      const reload = async () => {
        ...
        try {
          ...
          // 首次加载时若配置仍是 auto,把浏览器探测到的语言写回配置,
          // 使服务端消息(余额查询、价格同步等)与界面语言保持一致。

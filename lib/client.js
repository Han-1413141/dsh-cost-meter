...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.cm-peak-rail-classic{width:40px;height:46px;display:flex;flex-direction:column;align-items:stretch;justify-content:stretch;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;overflow:hidden;background:var(--dsw-alias-bg-layer-2);font-size:10px;line-height:1;box-sizing:border-box}',
      '.cm-peak-rail-classic-segment{position:relative;flex:1;display:flex;align-items:center;justify-content:center;writing-mode:vertical-rl;letter-spacing:1px;color:var(--dsw-alias-label-tertiary)}',
      '.cm-peak-rail-classic-segment>span{position:relative;z-index:1}',
      '.cm-peak-rail-classic-fill{position:absolute;top:0;left:0;width:100%;height:0;background:currentColor;opacity:.18;pointer-events:none}',
      '.cm-peak-rail-classic-segment.peak{background:color-mix(in srgb,#ff9800 18%,transparent);color:#ff9800}',
      '.cm-peak-rail-classic-segment.off{background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 14%,transparent);color:var(--dsw-alias-state-business-primary)}',
      '.cm-peak-rail-classic-segment.current{font-weight:700;outline:1px solid currentColor;outline-offset:-1px}',
      ...
      '.cm-catalog-vendor{margin-top:10px;font-size:12px;font-weight:600;color:var(--dsw-alias-label-tertiary);border-bottom:1px solid var(--dsw-alias-border-l1);padding-bottom:2px}',
    ...
    function parseSession(v, path) {
      ...
      return {
        ...
        byProviderModel: v.byProviderModel && typeof v.byProviderModel === 'object' ? v.byProviderModel : {},
      }
    }
     ...
     * 精确 → 去日期/版本后缀 → 候选前缀(最长) → 家族 token 相似(≥2 前缀 token,最长者胜)。
     */
    ...
    function resolveClientPrice(providerRaw, modelId, config) {
      ...
      if (hit === null) return { entry: null, priced: false, billingMode: 'flat' }
      return { entry: catalog[hit], priced: catalog[hit]?.unpriced !== true, billingMode: 'flat' }
    }
    ...
    /** 收起(rail)态经典样式:「峰时 / 平价」文字分段 + 当前段高亮 + 段内进度填充。 */
    function PeakRailStripClassic(props) {
      ...
      const view = peakView(config, now)
      if (view === null) return null
      const frac = view.nextAtMs > view.prevAtMs
        ? Math.max(0, Math.min(1, (now - view.prevAtMs) / (view.nextAtMs - view.prevAtMs)))
        : 0
      const fill = el('div', { className: 'cm-peak-rail-classic-fill', style: { height: (frac * 100).toFixed(1) + '%' } })
      const chipText = view.nextIntoPeak
        ? t('nextPeakIn', { time: countdownText(view, now, t) })
        : t('nextOffPeakIn', { time: countdownText(view, now, t) })
      const detail = [t(view.inPeak ? 'peakNotice' : 'offPeakActive'), chipText]
      return el(Tooltip, { label: detail, side: 'right', delayMs: 300 },
        el('div', { className: 'cm-peak-rail-classic', 'aria-label': detail.join('; ') },
          el('div', { className: 'cm-peak-rail-classic-segment peak' + (view.inPeak ? ' current' : '') },
            view.inPeak ? fill : null, el('span', null, t('peakShort'))),
          el('div', { className: 'cm-peak-rail-classic-segment off' + (view.inPeak ? '' : ' current') },
            view.inPeak ? null : fill, el('span', null, t('offPeakShort')))))
    }
          ...
          ? el(Fragment, null,
            ...
            providerIds.map(provider => {
              ...
              return el('div', { key: provider },
                ...
                }, (vendorOpen ? '▾ ' : '▸ ') + (CATALOG_VENDOR_LABELS[provider] ?? provider) + ' · ' + countModels(provider)),
                vendorOpen
                  ? el(Fragment, null,
                    ...
                    Object.keys(catalog[provider]).sort().map(family =>
                      el('div', { key: family },
                        ...
                        Object.keys(catalog[provider][family]).sort().map(modelId => {
                          const entry = catalog[provider][family][modelId]
    ...
    function CostSection(props) {
      ...
      return el('div', { className: 'cm-section' },
        ...
        // 已挂载的第三方 provider 模型(与 DeepSeek 同区展示,直接参与计费;来自拓展价格表的挂载)
        (() => {
          const providers = draft?.prices?.providers ?? {}
          const groups = Object.keys(providers)
            .filter(p => Object.keys(providers[p]?.models ?? {}).length > 0)
            .sort((a, b) => a.localeCompare(b))
          if (groups.length === 0) return null
          return groups.map(p =>
            el('div', { key: p },
              el('div', { className: 'cm-catalog-vendor' }, (CATALOG_VENDOR_LABELS[p] ?? p) + ' · ' + t('mountedSuffix')),
              Object.keys(providers[p].models).sort().map(id =>

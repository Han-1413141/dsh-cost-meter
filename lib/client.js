  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        planStatsNote: '口径:「金额」均为按目录 API 价折算的等值美元——订阅渠道消耗的是套餐额度,不动真金白银,预算/今日费用等金额展示只统计 API 渠道。每 1% 与满窗估算由相邻两次额度刷新采样的本地用量差 ÷ 百分比差推算(采样差分);样本不足时回退「本周期实际 ÷ 当前已用%」折算。5 小时窗为滚动窗,周/月按本地日历近似;各家仅返回百分比,无官方绝对量,估算仅供横向比较。SCNet 为本地 Credits 自估,不参与估算。',
      ...
      en: {
        ...
        planStatsNote: 'Scope: all amounts are catalog-price equivalents in USD — subscription channels consume plan quota, not real money; budget/today-cost displays only count the API channel. Per-1% and full-window values are derived from local usage deltas divided by percent deltas between consecutive quota-refresh samples (sample delta); when samples are insufficient they fall back to "this period actual ÷ current used %" (live). The 5-hour window is rolling; week/month follow the local calendar. Vendors only report percentages — no official absolute figures — so estimates are for cross-plan comparison. SCNet is a local credits self-estimate and does not participate.',

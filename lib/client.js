...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SidebarFooter(props) {
      ...
      if (!showBalance && !showToday) return null
      // 外壳的 footerActions 是横向 flex;这里用自建纵向堆叠保证余额在上、图框在下。
      return el('div', { className: 'cm-footer-stack' + (props.wide ? '' : ' rail') },
        showBalance ? el(BalanceRowContent, { state, wide: props.wide }) : null,
        showToday ? el(BudgetBoxContent, { state, wide: props.wide }) : null)
    }

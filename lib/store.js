...
import { DEFAULT_PLAN_PROVIDER_CLASS, PLAN_PROVIDER_IDS, billingClassOf, enabledPlanSetOf, planProviderIdOf, appendRecentCall, pruneRecentCalls } from './plan-billing.js'
...
/**
 * Plan/API 双轨拆分一次性迁移(issue #64):按当前分类配置回溯重算全部
 * day/session 的 apiCost(byProviderModel 条目按 provider:model 前缀分类,
 * plan 类条目金额不计入真金白银)。幂等:重复执行结果一致;分类随配置变化,
 * 迁移标记保证只跑一次——之后用户改动分类仅影响新调用。
...
export function splitLedgerApiCost(ledger) {
  ...
  const entryApiCost = (key, entry) => {
    const sep = key.indexOf(':')
    const provider = sep > 0 ? key.slice(0, sep) : key
    const model = sep > 0 ? key.slice(sep + 1) : key
    return billingClassOf(provider, model, planBilling, enabledPlans) === 'api' ? Number(entry?.cost) || 0 : 0
  }
  const containerApiCost = container => {
    const by = container?.byProviderModel
    if (by === null || typeof by !== 'object' || Object.keys(by).length === 0) {
      // 无模型明细的旧记录无法拆分:保持全部按 API 口径。
      return Number(container?.cost) || 0
    }
    let sum = 0
    for (const [key, entry] of Object.entries(by)) sum += entryApiCost(key, entry)
    return sum
  }
  let touched = 0
  for (const day of Object.values(ledger.days ?? {})) {
    if (day === null || typeof day !== 'object') continue
    const next = containerApiCost(day)
    if ((day.apiCost ?? -1) !== next) { day.apiCost = next; touched += 1 }
    if (!Array.isArray(day.sessions)) continue
    for (const session of day.sessions) {
      if (session === null || typeof session !== 'object') continue
      const sNext = containerApiCost(session)
      if ((session.apiCost ?? -1) !== sNext) { session.apiCost = sNext; touched += 1 }
    }
  }
  return touched
}
...
export class Ledger {
   ...
   * @param extras - 可选持久化附加状态({ planSamples, planRecentCalls })。
   */
  constructor(config, days, path, migrations = [], extras = {}) {
    ...
    // Plan 百分比采样历史与近期调用环形缓冲(issue #64)。
    this.planSamples = extras.planSamples ?? {}
    this.planRecentCalls = Array.isArray(extras.planRecentCalls) ? extras.planRecentCalls : []
  }
  ...
  static open() {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === 'object') {
        if (parsed.version !== LEDGER_VERSION) {
          ...
          // Plan 采样历史与近期调用缓冲(issue #64):形状不对则丢弃重建。
          ...
          if (Array.isArray(parsed.planRecentCalls)) planRecentCalls = parsed.planRecentCalls
        }
      }
    } catch (error) {
    ...
    const ledger = new Ledger(config, days, path, migrations, { planSamples, planRecentCalls })
  ...
  account(tokens, modelId, sessionId, atMs, provider) {
    ...
    // Plan 类调用同时进入近期调用环形缓冲(供 5 小时滚动窗本地量聚合)。
    const planBilling = this.config.planBilling
    const enabledPlans = enabledPlanSetOf(this.config)
    const cls = billingClassOf(provider, modelId, planBilling, enabledPlans)
    ...
    if (cls === 'plan') {
      ...
      if (planId !== null) {
        this.planRecentCalls = appendRecentCall(this.planRecentCalls, {
          t: atMs,
          provider: planId,
          tokens: buckets.input + buckets.output + buckets.cacheRead + buckets.cacheWrite + buckets.reasoning,
          cost,
        }, atMs)
      }
    }
  ...
  flush() {
    ...
    try {
      ...
      writeFileSync(tmp, JSON.stringify({
        ...
        planRecentCalls: pruneRecentCalls(this.planRecentCalls, Date.now()),
      }), 'utf8')

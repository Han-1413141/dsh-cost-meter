 ...
 *     宁多勿少(用户决定);
 *  4. modlens-zen 存量特判:该前缀历史桶归 go 订阅(apiCost=0);运行时别名表
 *     不含它——迁移后新调用照常按 api 计费(用户决定:之前的归订阅、以后归 api)。
...
export function splitLedgerApiCost(ledger) {
  ...
  // 存量分类器:modlens-zen 特判归 go(仅迁移重建用;account() 运行时不含此别名)
  const entryClass = (key, entry) => {
    const sep = key.indexOf(':')
    const provider = sep > 0 ? key.slice(0, sep) : key
    if (provider === 'modlens-zen') return 'plan'
    const model = sep > 0 ? key.slice(sep + 1) : key
    return billingClassOf(provider, model, planBilling, enabledPlans, prices)
  }
  let touched = 0
  for (const day of Object.values(ledger.days ?? {})) {
    ...
    for (const [key, entry] of Object.entries(day.byProviderModel ?? {})) {
      ...
      const nextApi = entryClass(key, entry) === 'api' ? cost : 0
      sumCost += cost
      sumApi += nextApi

...
import { DEFAULT_PLAN_PROVIDER_CLASS, PLAN_PROVIDER_IDS, billingClassOf, enabledPlanSetOf, planProviderIdOf, appendHourBucket, pruneHourBuckets, convertRecentCallsToBuckets } from './plan-billing.js'
...
export function repairLedgerPricing(ledger) {
  ...
  for (const [date, day] of Object.entries(ledger.days ?? {})) {
    ...
    for (const [key, bucket] of Object.entries(day.byProviderModel ?? {})) {
      ...
      const newApi = billingClassOf(provider, model, planBilling, enabledPlans) === 'api' ? newCost : 0
  ...
  flush() {
    ...
    try {
      ...
      writeFileSync(tmp, JSON.stringify({
        ...
        planHourBuckets: pruneHourBuckets(this.planHourBuckets, Date.now()),
      }), 'utf8')

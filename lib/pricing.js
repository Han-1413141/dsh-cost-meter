...
export function normalizePrice(value) {
  ...
  const legacyBase = tier(value.legacyBase)
...
export function tierFor(entry, atMs, peak) {
  ...
  if (Number.isFinite(atMs) && atMs < Date.parse(LEGACY_BASE_BOUNDARY)) {
    ...
    return lb === undefined
      ...
      : { cacheHit: lb.cacheHit, cacheMiss: lb.cacheMiss, output: lb.output }
  }
  ...
  if (isPeakHour(atMs, effectiveAtMs, peak.windows)) {
    ...
    return p === undefined
      ? { cacheHit: base.cacheHit, cacheMiss: base.cacheMiss, output: base.output }
      : { cacheHit: p.cacheHit, cacheMiss: p.cacheMiss, output: p.output }
  }
  if (effectiveAtMs !== undefined && atMs >= effectiveAtMs) {
    const off = base.offPeak
    return off === undefined
      ? { cacheHit: base.cacheHit, cacheMiss: base.cacheMiss, output: base.output }
      : { cacheHit: off.cacheHit, cacheMiss: off.cacheMiss, output: off.output }
  }

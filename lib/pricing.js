...
export function normalizePrice(value) {
  ...
  const entry = {
    cacheHit: num('cacheHit'),
    cacheMiss: num('cacheMiss'),
    output: num('output'),
  }
  if (value.legacy === true) entry.legacy = true
  const tier = raw => {
    if (raw === null || typeof raw !== 'object') return undefined
    return { cacheHit: num(raw.cacheHit), cacheMiss: num(raw.cacheMiss), output: num(raw.output) }
  }
...
export function parsePricingHtml(html) {
  ...
  if (Object.keys(models).length === 0) {
    throw new Error('官方页面中未解析出任何模型价格,页面结构可能已变化,请稍后重试或手动编辑价格')
  }

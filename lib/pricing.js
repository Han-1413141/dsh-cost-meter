...
function deepseekExplicitEntry(modelName, prices, mode) {
  const models = prices?.models ?? {}
  const hit = models[modelName] !== undefined
    ? modelName
...
export function parsePricingHtml(html) {
  ...
  // 中文页窗口为北京时间,提取后 -8h 折算为 UTC。
  const winZh = winEn === null ? /高峰时段为北京时间\s+(.+?)(?:（|\(|$)/.exec(plain) : null
